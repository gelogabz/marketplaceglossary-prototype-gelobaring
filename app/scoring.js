...
export function computeDifficulty(term) {
  ...
  if (/\b(billing|revenue|payout|disbursement|reconciliation|invoice)\b/.test(def)) score += 0.5;
  if (/\b(committed spend|drawdown|draw-down|macc|edp|ccs|draws)\b/.test(def)) score += 1.0;
  ...
  if (/\b(channel partner|resale|distributor|wholesale|markup|reseller|cppo|sppo|mcpo|mpo)\b/.test(def)) score += 0.75;
  if (/\b(multi-party|multiparty|co-sell|cosell|joint.sell)\b/.test(def)) score += 0.5;

  // Advanced / edge-case mechanics
  if (/\b(amendment|abo|flex commit|uplift|floor price|future dated|committed use discount)\b/.test(def)) score += 1.0;
  if (/\b(metering dimension|usage record|catalog api|fulfillment api|marketplace api)\b/.test(def)) score += 0.75;
  ...
  const isPlatformSpecific = tags.some((t) => ["aws", "azure", "gcp", "snowflake", "alibaba"].includes(t));
  if (!isPlatformSpecific && tags.includes("general")) score = Math.max(0, score - 0.4);
