...
import { buildInlineTermDetail, injectTagStyles, slug, copyToClipboard } from '../app/render.js';
...
function renderPath(path) {
    ...
    const sameCatPaths = learningPaths.filter(p => p.category === path.category && p.slug !== path.slug);
    const nextPath = sameCatPaths[0] || null;

    // Progress dots HTML
    const dotsHtml = path.steps.map((_, i) =>
